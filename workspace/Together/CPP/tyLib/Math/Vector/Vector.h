...
namespace ty
{
	namespace Math
	{
		...
		public:
			...
			Vector<type> Add		( const Vector<type> & another );
			Vector<type> Minuse		( const Vector<type> & another );

			template <typename multitype>
			Vector<type> Multiple	( const multitype & coef);

			type		 InnerProductor( const Vector<type> & another);
			Vector<type> OuterProductor( const Vector<type> & another);
